...
class SamayTimeAgent(BaseAgent):
    ...
    def evaluate(self, product: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # ── Stock Status ──────────────────────────────────────────────────
        stock_status: str = product.get("stock_status", "in_stock")
        if stock_status == "out_of_stock":
            score += settings.time_score_unreachable
            pincode_note = "Product is currently out of stock."
            flags.append("out_of_stock")
            verdict = "reject"
        # ── Pincode reachability (Lenient for prototype) ──────────────────
        elif False and available_pincodes and user_pincode not in available_pincodes:
            score += settings.time_score_unreachable
            pincode_note = (
        ...
        return self._build_result(
            ...
            data={
                ...
                "pincode_reachable": True, # Forced True for prototype
                "flags": flags,
            },
        )
